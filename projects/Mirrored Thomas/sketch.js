...
function draw() {
  ...
  let b = 0.19;
  let t = map(noise(frameCount * 0.001 + 1), 0, 1, 0.01, 0.05);
  let ball_max_size = 0.5;
  ...
  pg.pointLight(
    ...
    0);
  pg.rotateY(sin(frameCount * 0.001) * TAU);
  pg.rotateZ(PI/2)
  for (let i = 0; i < sp.length; i++) {
    ...
    let nx = p[0] +(-b*p[0]+sin(p[1])) * t;
    let ny = p[1] +(-b*p[1]+sin(p[2])) * t;
    let nz = p[2] +(-b*p[2]+sin(p[0])) * t;
    sp[i][0] = nx;
    sp[i][1] = ny;
    sp[i][2] = nz;
